...
struct PCB{
    ...
    int             priority;        // lower = higher priority (for EP, EP_RR)
    unsigned int    time_to_next_io; // counts down while RUNNING
    unsigned int    wake_time;       // when I/O completes (for WAITING → READY)
    unsigned int    finish_time;     // when process TERMINATED (for metrics)
};
...
PCB add_process(std::vector<std::string> tokens) {
    ...
    process.state          = NEW;              // instead of NOT_ASSIGNED if you want
    process.time_to_next_io= (process.io_freq > 0 ? process.io_freq : 0);
    process.wake_time      = 0;
    process.finish_time    = 0;
    process.priority       = process.PID;      // simple external priority
